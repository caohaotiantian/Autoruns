...
namespace Autoruns
{
    class DriversStartups : Startups
    {
        private const string RegEntry = "HKLM\\System\\CurrentControlSet\\Services";

        public DriversStartups(ListView _listView)
        {
            listView = _listView;
            LoadRegEntry();
        }
            ...
            foreach (string appEntry in subKeyNames)
            {
                ...
                if (k == null || k.GetValue("serviceDLL") == null)
                {
                    if (appSubKey.GetValue("ImagePath") != null)
                    {
                        ...
                        if (appSubKey.GetValue("DisplayName") != null)
                            disp = GetValueContentAsPath(appSubKey.GetValue("DisplayName").ToString());

                        if (target1 == "") continue;
                        if (GetFileDescription(target1) == "") continue;
                        AddStartupEntry(false,
                            appEntry,
                            disp + GetFileDescription(target1),
